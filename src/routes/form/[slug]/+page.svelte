<script>
	...
	let customForm = $derived(data.customForm);
	let products = $derived(data.products ?? []);
	let globalAddons = $derived(data.globalAddons ?? []);
	let siteInfo = $derived(data.siteInfo);
	let bannerText = $derived(data.bannerText || customForm?.title || i18n.t('orderForm.bannerDefault'));
...
<svelte:head>
	<title>{customForm?.title || i18n.t('orderForm.title')}</title>
	<meta name="description" content={customForm?.description || i18n.t('orderForm.subtitle')} />
</svelte:head>
		...
		<div class="mb-8 rounded-3xl border border-primary/15 bg-white p-6 sm:p-8 shadow-sm shadow-primary/5 relative overflow-hidden">
			...
			<div class="relative z-10 max-w-2xl">
				...
				<h1 class="font-serif text-3xl font-bold tracking-tight text-[#4A3B32] sm:text-4xl">
					{customForm?.title || i18n.t('orderForm.title').split('|')[0].trim()}
				</h1>
				<p class="mt-2 text-sm sm:text-base text-[#4A3B32]/70">
					{customForm?.description || i18n.t('orderForm.subtitle')}
				</p>
			</div>
		</div>
			...
			<div class="lg:col-span-8 space-y-6">
				...
				<section class="rounded-3xl border border-primary/10 bg-white p-6 sm:p-8 shadow-sm">
					<div class="flex items-center justify-between border-b border-primary/10 pb-4 mb-6">
						...
						{#if selectedProduct && !isChangingProduct && products.length > 1 && !customForm?.product_id}
							<button
								type="button"
					...
					{#if isChangingProduct && products.length > 1 && !customForm?.product_id}
